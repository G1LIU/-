...
void OLED_ShowSignedNum(uint8_t x, uint8_t y, int32_t num, uint8_t len)
{
    ...
    for(t = 0; t < len; t++)
    {
        uint8_t temp = (abs_num / OLED_Pow(10, len - t - 1)) % 10;
        OLED_ShowChar(x + (t + 1) * 8, y, temp + '0');
    }
}
...
void OLED_ShowHexNum(uint8_t x, uint8_t y, uint32_t num, uint8_t len)
{
    ...
    OLED_ShowString(x, y, "0x");  // 显示前缀

    for(t = 0; t < len; t++)
    {
        temp = (num / OLED_Pow(16, len - t - 1)) % 16;
        if(temp < 10) OLED_ShowChar(x + (t + 2) * 8, y, temp + '0');
        else OLED_ShowChar(x + (t + 2) * 8, y, temp - 10 + 'A');
    }
}
